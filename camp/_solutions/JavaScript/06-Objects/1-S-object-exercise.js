...
function authorsByYear(books) {
  ...
  for (let book of books) {
    if (result[book.read] === undefined) {
      result[book.read] = [book.author];
    }
    else if (result[book.read].indexOf(book.author) === -1) {
      result[book.read].push(book.author);
      result[book.read].sort();
    }
  }
